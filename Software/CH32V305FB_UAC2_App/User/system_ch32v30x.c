...
#define SYSCLK_FREQ_120MHz_HSE  122880000
...
static void SetSysClockTo120_HSE(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->CFGR0 &= (uint32_t)((uint32_t)~(RCC_PLLSRC | RCC_PLLXTPRE |
        ...
        //PLL2_DIV = 5
        RCC->CFGR2 = ( RCC->CFGR2 &( ~RCC_PREDIV2 )) | ( ( 5 - 1 ) << 4 );
        //PLL2_MUL = 16
        RCC->CFGR2 = ( RCC->CFGR2 &( ~RCC_PLL2MUL )) | ( 14 << 8 );
        RCC->CTLR |= RCC_PLL2ON;
        ...
        //PLL_PRE_DIV = 10
        RCC->CFGR2 = ( RCC->CFGR2 &( ~RCC_PREDIV1 )) | ( 10 - 1 );
        //Select PREDIV1 input
        RCC->CFGR0 |= RCC_PLLSRC_HSE ;
        //PLL_MUL  = 16 , 122880000Hz
        RCC->CFGR0 = ( RCC->CFGR0 &( ~RCC_PLLMULL )) | ( RCC_PLLMULL16_EXTEN );
#endif
