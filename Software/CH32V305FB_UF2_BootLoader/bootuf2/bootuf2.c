...
static void bootuf2block_state_update(struct bootuf2_STATE *STATE,
                                      struct bootuf2_BLOCK *uf2, uint32_t block_max)
{
    USB_LOG_INFO("UF2 block total %d--%d written %d index %d\r\n",
                uf2->NumberOfBlock, STATE->NumberOfBlock, STATE->NumberOfWritten, uf2->BlockIndex);
    if (uf2->NumberOfBlock) 
    {
        if (STATE->NumberOfBlock != uf2->NumberOfBlock) 
        {
            if ((uf2->NumberOfBlock >= BOOTUF2_BLOCKSMAX) || STATE->NumberOfBlock ) 
            {
...
{
    return (STATE->NumberOfWritten >= STATE->NumberOfBlock) && STATE->NumberOfBlock;
}
...
int bootuf2_flash_write_internal(struct bootuf2_data *ctx, struct bootuf2_BLOCK *uf2)
{
    ...
    /*!< 2.cache full */
    if ((ctx->cached_bytes && ((ctx->cached_address + ctx->cached_bytes) != uf2->TargetAddress)) || (ctx->cached_bytes == ctx->cache_size ) ) 
    {
        USB_LOG_INFO("bootuf2_flash_write_internal.\r\n");
        int err = bootuf2_flash_flush(ctx);
        if (err)
            return err;
    }
    //USB_LOG_INFO("bootuf2_finished.\r\n");
...
int bootuf2_write_sector(uint32_t start_sector, const uint8_t *buff, uint32_t sector_count)
{
    ...
    while (sector_count) 
    {
        ...
        //USB_LOG_INFO("MagicStart0=0x%08X.\r\n",uf2->MagicStart0);
        //USB_LOG_INFO("MagicStart1=0x%08X.\r\n",uf2->MagicStart1);
        //USB_LOG_INFO("MagicEnd=0x%08X.\r\n",uf2->MagicEnd);
        //USB_LOG_INFO("BlockIndex=0x%08X.\r\n",uf2->BlockIndex);
        //USB_LOG_INFO("TargetAddress=0x%08X.\r\n",uf2->TargetAddress);
        //USB_LOG_INFO("Flags=0x%08X.\r\n",uf2->Flags);
        ...
        if (uf2->FamilyID == CONFIG_BOOTUF2_FAMILYID) 
        {
            ...
            {
                //USB_LOG_INFO("Vaild UF2 block.\r\n");
                bootuf2_flash_write_internal(ctx, uf2);
                bootuf2block_state_update(ctx->STATE, uf2, CONFIG_BOOTUF2_FLASHMAX );
            } 
            else 
            {
                USB_LOG_INFO("UF2 block %d already written\r\n",
                            uf2->BlockIndex);
            }
        } 
        else 
        {
            USB_LOG_INFO("UF2 block illegal id %08x\r\n", uf2->FamilyID);
        }

    next:
        //USB_LOG_INFO("None UF2 block.\r\n");
        start_sector++;
        sector_count--;
        buff += ctx->DBR->BPB.BytesPerSector;
    }
...
{
    //USB_LOG_INFO("UF2 block total %d written %d.\r\n", bootuf2_disk.STATE->NumberOfBlock , bootuf2_disk.STATE->NumberOfWritten );
    if (bootuf2block_state_check(bootuf2_disk.STATE)) 
    {
        bootuf2_flash_flush(&bootuf2_disk);
        USB_LOG_INFO("UF2 update ok\r\n");
        return true;
    } 
    else 
    {
